...
public interface OrderMapper {
    ...
    /** DAL-001：poiId 声明了但 XML 没用。 */
    List<Object> queryOrder(@Param("merchantId") Long merchantId,
                            @Param("poiId") Long poiId,
                            @Param("status") String status);

    /** 无 @Param 双参数：XML 用 #{param2} 覆盖了 status，merchantId 未用 → DAL-001（中置信度）。 */
    ...
    /** 单 Bean：展开属性。pageNum / pageSize 被内置分页名单忽略，remark 未用 → DAL-001（低置信度，定位到 OrderQuery 字段）。 */
