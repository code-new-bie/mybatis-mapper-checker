...
上游赋值分析（默认开）：DAL-001 / DAL-010 报出来后，再看一眼"上游到底有没有真的给过值"——调用点传了非 null 的实参 / setter 值，说明值被静默丢弃，置信度上调。找到的赋值全是字面量 `null`：DAL-010 置信度下调（仍照常报告）；DAL-001 直接判定为无害、不计入问题（统计条"已排除：N"），因为 DAL-001 担心的是"有值却没用上"，确认过从来没给过真值，SQL 不用它就完全说得通。

实体属性会**沿调用链往上追**到对象真正被构造的地方：Spring 分层里 Query 大多在 Service / Controller 层 `new` 出来、当参数一层层传到 DAO，只看 DAO 的直接调用点是追不动的。往上最多追 4 层，中间层经接口调用（`@Autowired XxxService svc; svc.method(query)`）也能接上。因此 Query 类被多方法共用时，判断的是"这个 DAO 方法自己这条链上有没有赋过值"，不会被同一个 Query 类其他方法的赋值误伤。

追不出结论的（对象由 Builder / 跨方法构造返回、调用链超过深度上限、调用点在扫描范围之外、值可能由 `BeanUtils.copyProperties` 之类反射拷贝填入）不下结论，也不跟已确认的问题混在一起：报告里单独一组"证据不足，待人工确认"，默认折叠，选中分组会说明为什么追不出来；导出的 Markdown 有同名章节，CSV 状态列写"证据不足"。判"确认没赋过值"比判"赋过值"严格得多——前者会把问题静默排除掉，只有这条链上每个调用点都追出确定结论才敢下这个判断。
...
结果在底部工具窗口 `MyBatis Mapper Checker` 中，分「问题」「证据不足，待人工确认」「无法解析」「已豁免」四组，前两组内部按 Module → 文件分层，带置信度与备注。双击跳到 Java 位置，右键可跳 Mapper、豁免此处、忽略此处 / 参数 / statement、查看该处提交信息、查看调用链、导出 Markdown / CSV。工具栏"显示责任人"（默认关，需要项目在版本控制下且装了对应 VCS 插件如 Git4Idea）打开后，每条问题后面会带上最后修改它的作者与日期，方便按人分派。
